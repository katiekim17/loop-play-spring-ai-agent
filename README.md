...
LLM을 쓰는 코드를 처음 짜보면서 가장 인상 깊었던 건 Spring AI의 `.entity(SupportResponse.class)`가 동작하는 방식이었다. Java 클래스 정의만 넘겨주면 Spring AI가 JSON 스키마를 자동으로 만들어 System Prompt 끝에 주입하고, LLM이 그 스키마에 맞춰 응답을 생성한다. "프레임워크가 프롬프트를 대신 써준다"는 감각이 신기했다. 그게 4단계 토큰 실험에서 직접 확인됐다. 프롬프트를 2배로 늘렸는데 토큰은 65%만 늘어난 이유가, 원래 748 토큰 중 일부는 이미 Spring AI가 주입한 스키마였기 때문이었다.

두 번째는 "구조화 프롬프트가 항상 더 낫지 않다"는 것이다. 시나리오 3(라이더 음식 엎음)에서 단순 프롬프트 consistency 1.00, 구조화 프롬프트 0.60이 나왔다. 지시가 많아질수록 모델이 더 많은 요소를 고려해서 오히려 일관성이 떨어질 수 있다. 이걸 실험으로 직접 보기 전까지는 "더 자세히 써줄수록 좋은 것"이라고 막연하게 생각했다.

세 번째는 [금지] 규칙의 효과다. 규칙을 빼도 LLM이 표면적으로는 개인정보 노출을 거절했다. 그런데 거절의 성격이 달랐다. 규칙이 있을 때는 "금지되어 있습니다(정책 원칙)"로 거절하고, 없을 때는 "앱에서 불가능합니다(기술적 제약)"로 거절했다. 같은 거절처럼 보여도 모델이 근거로 삼는 것이 달라서, 다른 상황에서는 다르게 행동할 수 있다는 걸 알았다.

### 의문점

**구조화 프롬프트 consistency가 왜 낮아졌나.** 시나리오 3에서 구조화 프롬프트가 COMPLAINT×3, DELIVERY×2로 흔들렸는데, 지시가 많아지면 항상 일관성이 낮아지는 건지, 아니면 이 특정 시나리오에서만 그런 건지 모르겠다. 10회, 20회로 반복 수를 늘리면 수치가 달라질지 궁금하다.

**Spring AI가 주입하는 JSON 스키마의 정확한 내용.** 4단계에서 실제 프롬프트 구조를 추정해서 README에 적었는데, Ollama에 실제로 전달되는 전체 프롬프트를 raw로 볼 수 있는 방법이 있을 것 같다. `TRACE` 로그나 Ollama 요청 로그로 확인 가능한지 아직 못 찾았다.

**Streaming 중에 부분적인 구조화 파싱이 가능한가.** 현재 `.stream()` + `.entity()`는 같이 쓸 수 없다. 그런데 `summary` 필드는 먼저 화면에 보여주면서 나머지 필드는 뒤에 채우는 방식이 가능할 것 같다. JSON을 완전히 받기 전에 부분 파싱하는 라이브러리나 패턴이 있는지 궁금하다.

### 다음 주차에 시도하고 싶은 것

2주차가 Tool Calling이라면, 지금 만든 상담 에이전트에 실제로 주문을 조회하는 Tool을 붙여보고 싶다. 지금은 "주문번호 2024-1234 어디 있어요?"라고 물으면 LLM이 `NEEDS_INFO`로 분류하고 "주문번호를 알려주시면..."으로 답한다. 그런데 Tool Calling이 연결되면 주문번호가 있을 때 실제 주문 상태 API를 호출해서 "현재 강남구 삼성동 근처입니다"처럼 데이터 기반으로 답변할 수 있을 것이다.

그리고 지금 PerformanceLoggingAdvisor가 LLM 호출 시간만 측정하는데, Tool 호출이 추가되면 "LLM 추론 시간 vs Tool 호출 시간" 비율도 로깅하면 흥미로울 것 같다. 어떤 문의가 외부 API를 많이 쓰는지 패턴을 볼 수 있을 테니까.
