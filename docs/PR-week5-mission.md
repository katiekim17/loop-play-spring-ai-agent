...
- [ ] **공통 (10점)** — 학습 기록 3단락 (아래 템플릿, 검증 후 작성)
...
## 공통 학습 기록 (검증 후 작성 템플릿)

- **내가 배운 것** — (예: 다층 방어 / Advisor order 세분화 / short-circuit 비용 0 / 규칙 기반의 한계 중 직접 겪은 것)
- **의문점** — (예: 분류 LLM을 앞에 두면 비용/지연이 얼마나? / 민감정보 정규식 국가별 유지 / Handoff 이후 실제 상담원 시스템 연결 트리거)
- **Round 6에 시도하고 싶은 것** — (예: 차단 카운터를 Micrometer 메트릭으로 / Handoff 시 대화 요약 전달 / 분류 LLM으로 규칙 보강)
